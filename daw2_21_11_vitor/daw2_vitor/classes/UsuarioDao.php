...
class UsuarioDao extends Db implements InterfaceDao {
    ...
    public function insert($usuario) {
        $stmt = $this->conexao->prepare("INSERT INTO {$this->table} (nome, senha, email, criptografia) VALUES (:nome, :senha, :email, :criptografia)");

        $stmt->bindValue(':nome', $usuario->getNome());
        $stmt->bindValue(':senha', $usuario->getSenha());
        $stmt->bindValue(':email', $usuario->getEmail());
        $stmt->bindValue(':criptografia', $usuario->getCriptografia());
    ...
    public function update($usuario) {
        $stmt = $this->conexao->prepare("UPDATE {$this->table} "
                . "SET nome=:nome, senha = :senha, email = :email, criptografia = :criptografia WHERE id = :id;");
    ...
    public function selectById($usuario) {
        ...
        $usuario->setSenha($linha['senha']);
        $usuario->setEmail($linha['email']);
        $usuario->setCriptografia($linha['criptografia']);
        $usuario->setId($linha['id']);
